...
class InstallerWindow:
    ...
    def build_hdds(self):
        ...
        model2 = gtk.ListStore(str,str,str,str)            
        ...
        for word_driver in driver_informations:
            if ( id_index == 0 ):
                ...
                fdisk_output = commands.getoutput("fdisk -l 2>/dev/null")
                fdisk_output_disks  = fdisk_output.split("\n\n");
                for fdisk_output_disk in fdisk_output_disks:
                    if((id_name in fdisk_output_disk) and ("identifier" in fdisk_output_disk) and not("0x00000000" in fdisk_output_disk) ):
            ...
            if ( size_index == 0 ):
                ...
                size_index = 1
                disk_number += 1
                iter = model2.append(["Disk "+str(disk_number),model_name,partition_number,size]);
    ...
    def build_grub_partitions(self):
        ...
        # Add disks and corresponding spaces
                ...
                while (partition is not None):
                    ...
                    if last_added_partition.partition.number == -1 and partition.number == -1:
                        ...
                        if "swap" in last_added_partition.type:
                          ...
                          #  print(str(last_added_partition.free_space))
                    ...
                    if last_added_partition.size > 1.0:
                        ...
                        region_number += 1
                        if str(last_added_partition.free_space).isdigit() or last_added_partition.type == "swap"  :                           
                        ...
                        #    iter = model.append([display_name, last_added_partition.type, last_added_partition.description, str('%.0f' % round(last_added_partition.size, 0)) + " MB", str(last_added_partition.free_space), last_added_partition, False, last_added_partition.start, last_added_partition.end, False]);
                            ...
                            if last_added_partition.partition.number == -1:    
                                ...
                                if last_added_partition.type == "ntfs":
                                    ...
                                    color = "#a9a9a9"
                                if last_added_partition.type != "swap":
    ...
    def show_overview(self):
        ...
        if(self.setup.grub_device is not None):
            ...
            grub_path= self.setup.grub_device.split(" ",1);
            self.setup.grub_device = grub_path[0] 
        else:
        ...
        for partition in self.setup.partitions:
            target_path = " ( "
            target_path += self.setup.target_disk
            target_path += " : "
            target_path += partition.display_name
            target_path += " ) "
            if(partition.mount_as is not None and partition.mount_as != "None"):
                # mount point
                iter = model.append(top)
                model.set(iter, 0, "<b>%s</b>" % (_("Mount %(partition)s as %(mountpoint)s") % {'partition':target_path, 'mountpoint':partition.mount_as}))

            if(partition.format_as is not None and partition.format_as != "None"):
                iter = model.append(top)
                model.set(iter, 0, "<b>%s</b>" % (_("Format %(partition)s as %(format)s") % {'partition':target_path, 'format':partition.format_as}))
