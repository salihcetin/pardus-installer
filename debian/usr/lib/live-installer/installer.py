...
class InstallerEngine:
            ...
            if(partition.format_as is not None and partition.format_as != ""):                
                ...
                if partition.format_as == "swap":
                    ...
                    if (partition.format_as == "jfs"):
                        ...
                        cmd = "mkfs.%s -t %s %s" % (partition.format_as, partition.format_as, partition.partition.path) # works with bfs, btrfs, ext2, ext3, ext4, minix, msdos, ntfs, vfat
    ...
    def install(self, setup):        
        ...
        try:
            ...
            our_current += 1
            self.update_progress(pulse=True, total=our_total, current=our_current, message=_("Finishing installation"))
